...
- **Attribute-level conformance**. `tests/attribute_conformance.rs`
  ...
  implementation — see [Building from source](#building-from-source).
...
The published crate is pure-Rust: depending on `sdef` from `Cargo.toml`
pulls in `quick-xml`, `serde`, and `thiserror` only.
